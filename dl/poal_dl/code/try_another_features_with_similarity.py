...
def get_features(labeled_in_loader, model):
    model.eval()
    ...
    for data in tqdm(labeled_in_loader):
        inputs = data[0].to(cuda_device)
        _, couts = model(inputs, **kwargs)
        features_in = torch.cat((features_in, couts['simclr'].detach()), 0)
    return features_in

def get_CSI_score(features_in, features_unlabeled):
    # CSI Score
    sim_scores = torch.tensor([]).to(cuda_device)
    ...
    return sim_scores.type(torch.float32).cpu().reshape((-1, 1))
...
while (iteration > 0):
    ...
    for r in range(NUM_ROUND):
        ...
        labeled_features = get_features(label_loader_without_train, model_csi)
        unlabeled_features = get_features(unlabed_loader_without_train, model_csi)

        purity = get_CSI_score(labeled_features, unlabeled_features)

        Maha_score = standardize(purity)[0].numpy()
        ...
        inv_Maha_score = Maha_score # np.max(Maha_score) - Maha_score # centered的M score 相对的
